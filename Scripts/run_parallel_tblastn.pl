...
foreach my $i (@numfiles) {
    my $pid = fork();
    if ($pid==0) { # child
        exec("tblastn -query $query\_$i\.tmpfa -db $database -out $query\_$i.blastout -outfmt \"6 std sframe qlen slen\" -evalue $eval");
        die "ERROR in run_parallel_tblastn.pl Exec $i failed: $!\n";
    } elsif (!defined $pid) {
        warn "ERROR in run_parallel_tblastn.pl Fork $i failed: $!\n";
    }
}
