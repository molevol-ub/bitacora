...
parseAndStoreGFF();
sortAndCheck();
open MRNA, ">$mrnafile" or die ("ERROR in gffGetmRNA.pl: Could not open mrnafile for writing.");
printmrnas();
close MRNA;
if (keys %ignorechr > 0){
...
sub readGenome {
    my $seqname;
    if ($seqfile){
	open (SEQ, "<$seqfile") or die ("ERROR in gffGetmRNA.pl: Could not open sequence file $seqfile\n");
	$/=">";
	while (<SEQ>){
	    s/>$//;
	    next unless /\S+/;
...
sub sortAndCheck {
	...
	foreach my $txid (sort keys %{$txs{$chr}}){
		...
		if (!defined($strand)){
		    ...
		    die ("ERROR in gffGetmRNA.pl: Strand not consistent in transcript $txid.") if ($strand ne $exon->[6]);
		}
		if (defined($end) && $end >= $exon->[3]){
		    die ("ERROR in gffGetmRNA.pl: Exons overlapping in $txid.");
		}
	    }
	}
    }
...
sub printmrnas() {
	...
	if ($sequence{$chr}){
		...
		my $seq = "";
		foreach my $exon (@{$txs{$chr}{$txid}}){
		    die ("ERROR in gffGetmRNA.pl: Coordinate of $txid out of sequence range. $exon->[4] >= " . length($sequence{$chr}) . "\n") 
