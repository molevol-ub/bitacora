...
# usage: perl get_nr_gff.pl seqs_nr.fasta (annnotated.gff or genomic.gff) outname
...
open (Results, ">", "$ARGV[2]\_genomic_and_annotated_genes_nr.gff3");
...
while (<GFFfile>) {
	...
	if ($subline[2] =~ /CDS/){
		...
		else {die "ERROR in get_nr_gff_only1gff.pl: It fails detecting Parent ID in $line\n";}
		...
		else {print "ERROR in get_nr_gff_only1gff.pl: It fails detecting ID in $line\n";}
