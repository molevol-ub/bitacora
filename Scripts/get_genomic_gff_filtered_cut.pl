...
# usage: perl get_genomic_gff_filtered_cut.pl X_genomic_genes_unfiltered.gff3 $genome X_genomic_geneshmmer_parsed_list.txt outname
...
while (<GFFfile>) {
	...
	if ($subline[2] =~ /CDS/){
		...
		else {die "ERROR in get_genomic_gff_filtered_cut.pl: It fails detecting Parent ID in $line\n";}
		...
		else {print "ERROR in get_genomic_gff_filtered_cut.pl: It fails detecting ID in $line\n";}
		...
		if (exists $gffgene{$genename}){ # Control for duplicated genes in GFF3
			die "ERROR in get_genomic_gff_filtered_cut.pl: Gene $genename is duplicated in the GFF3, found duplicate in $line\nPlease, take a look into your GFF3 and delete duplicated genes\n";
		}
...
###
	...
	} else {die "ERROR in get_genomic_gff_filtered_cut.pl: It fails finding the gene in GFF $subline[0]\n";}
...
open (Results, ">", "$ARGV[3]\_genomic_genes_cut.gff3");
...
while (<File>) {
	...
	if (exists $gffgene{$gene}){
		...
		if ($subl2[6] =~ /\+/){ # Forward strand
			...
			@posorder = sort { $b <=> $a } @positions;
		} else {die "ERROR in get_genomic_gff_filtered_cut.pl: No forward/reverse in $gffgene{$gene}\n";}
		...
		foreach my $cds (@{$gffcds{$gene}{$posit}}) {
			...
			if ($subl3[8] =~ /ID=([^;]+)/){
				$cdsid = $1;
			} else {die "ERROR in get_genomic_gff_filtered_cut.pl: Can't find CDS name in $cds\n";}
			...
			#}
				...
				die "ERROR in get_genomic_gff_filtered_cut.pl: Debugging control: Incongruence observed in (some situation not considered):\n$line\n$cds\ninicuted:$inicuted\nendcuted:$endcuted\n";
			}
		...
		} else {die "ERROR in get_genomic_gff_filtered_cut.pl: Cant't find strand in GFF3: $gffgene{$gene}\n";}
	...
	} else {die "ERROR in get_genomic_gff_filtered_cut.pl: Protein gene $gene $subline[0] is not found in the GFF3\n";}
...
#system ("perl $dirname/gff2fasta_v2.pl $genome $ARGV[3]\_genomic_genes_cut.gff3 $ARGV[3]gffgenomiccut");
#system ("perl $dirname/reconvert_fasta.pl $ARGV[3]gffgenomiccut\.pep.fasta");
system ("perl $dirname/gff2fasta_v3.pl $genome $ARGV[3]\_genomic_genes_cut.gff3 $ARGV[3]gffgenomiccut");
