...
foreach my $i (@numfiles) {
    my $pid = fork();
    if ($pid==0) { # child
        exec("blastp -query $query\_$i\.tmpfa -db $database -out $query\_$i.blastout -outfmt \"6 std qlen slen\" -evalue $eval");
        die "ERROR in run_parallel_blastp.pl Exec $i failed: $!\n";
    } elsif (!defined $pid) {
        warn "ERROR in run_parallel_blastp.pl Fork $i failed: $!\n";
    }
}
