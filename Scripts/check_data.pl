	...
	if ($line =~ />(\S+)/){
		...
		if (exists $gffgene{$gene}){
			...
			if (exists $gffgene{$genenew}){
				...
				die "ERROR in $dirname/check_data.pl: Protein gene $gene nor $genenew is not found in the GFF3\nIf your GFF comes from NCBI, you probably need to reformat the GFF to assign protein ID in mRNA and CDS fields. For that, you can use the following Script and input the formatted GFF in PAIP\nperl $dirname/Tools/reformat_ncbi_gff.pl $gff\n\nOtherwise, if this error keeps showing, You can run the following script to check which proteins are not annotated in the GFF, or contain different name in mRNA|transcript field in the GFF to explore the data. You can use the outfile_ingff.fasta tu use only annotated sequences in PAIP\nperl $dirname/Tools/get_proteins_notfound_ingff.pl $gff $proteome\n\nFinally, if all your proteins are named different as in the GFF3, you can codify the protein sequences directly from your GFF3 file with the following script, and use the resulting proteins in PAIP:\nperl $dirname/gff2fasta_v3.pl $genome $gff $gff\n\n";
			}
