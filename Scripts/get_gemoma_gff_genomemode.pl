...
open (Results, ">", "$outdir\_gemoma_genes.gff3");

my $genecount = "000";

foreach my $key (keys %gffgene){
	my $gene = "$key";
	...
	if (exists $gffgene{$gene}){
		...
		if ($subl2[8] =~ /ID=[^;]+;(\S+)/){
			...
			die "ERROR in get_gemoma_gff_genomemode.pl: Could not find description after ID in $gffgene{$gene}\n";
		}
	...
	} else {die "ERROR in get_gemoma_gff_genomemode.pl: It fails finding the gene in GFF $gene\n";}
...
system ("perl $dirname/gff2fasta_v3.pl $genome $outdir\_gemoma_genes.gff3 $outdir\_gemoma_genes");
