...
# usage: perl get_tblastn_parsed_newv_gff_genomic_positions.pl outfmt6_file outname gff_file evalue
...
while (<Blastfile>) {
	...
	$scaflength{$subline[1]} = $subline[23];
	...
	if ($subline[19] =~ /-1/){ 
		...
		my $left = $ini;
		my $right = $fin;
		push (@{$blast{$subline[1]}}, join("\t",$subline[0], $subline[10], $subline[3], $subline[23], $subline[22], $frame, $left, $right, $subline[2]));
	}
	elsif ($subline[19] =~ /-2/){
		$frame = 5;
		my $left = $ini;
		...
		push (@{$blast{$subline[1]}}, join("\t",$subline[0], $subline[10], $subline[3], $subline[23], $subline[22], $subline[19], $subline[8], $subline[9], $subline[2]));
	}	
}
close Blastfile;
...
foreach my $key (sort keys %hits) {
	foreach my $position (@{$hits{$key}}){
		...
		if ($frame == 1 || $frame == 2 || $frame == 3){
			...
			$cadena = "R";
		} else {die "ERROR in get_genomic_tblastn_parsed_newv_gff_genomic_positions.pl: Can't find frame in $position\n";}
