...
while (<GFFfile>) {
	...
	if ($subline[2] =~ /CDS/){
		...
		else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: It fails detecting Parent ID in $line\n";}
		...
		else {print "ERROR in get_genomic_gff_filtered_trimmed.pl: It fails detecting ID in $line\n";}
		...
		if (exists $gffgene{$genename}){ # Control for duplicated genes in GFF3
			die "ERROR in get_genomic_gff_filtered_trimmed.pl: Gene $genename is duplicated in the GFF3, found duplicate in $line\nPlease, take a look into your GFF3 and delete duplicated genes\n";
		}
...
###
	...
	} else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: It fails finding the gene in GFF $subline[0]\n";}
...
while (<File>) {
	...
	if (exists $gffgene{$gene}){
		...
		if ($subl2[6] =~ /\+/){ # Forward strand
			...
			@posorder = sort { $b <=> $a } @positions;
		} else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: No forward/reverse in $gffgene{$gene}\n";}
		...
		foreach my $cds (@{$gffcds{$gene}{$posit}}) {
			...
			if ($subl3[8] =~ /ID=([^;]+)/){
				$cdsid = $1;
			} else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: Can't find CDS name in $cds\n";}
			...
			#}
				...
				die "ERROR in get_genomic_gff_filtered_trimmed.pl: Debugging control: Incongruence observed in (some situation not considered):\n$line\n$cds\ninicuted:$inicuted\nendcuted:$endcuted\n";
			}
		...
		} else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: Cant't find strand in GFF3: $gffgene{$gene}\n";}
	...
	} else {die "ERROR in get_genomic_gff_filtered_trimmed.pl: Protein gene $gene $subline[0] is not found in the GFF3\n";}
