...
foreach my $chem (@chemosensory){
	...
	close FileGemo;
	if ($gemoerr > 0){
		die "ERROR in $dirname/runanalysis_2ndround_v2_genomic_withgff_gemoma: GeMoMa died with error running GeMoMa\n";
	}
	...
	close FileGemo;
	if ($gemoerr > 0){
		die "ERROR in $dirname/runanalysis_2ndround_v2_genomic_withgff_gemoma: GeMoMa died with error runnning GAF\n";
	}
	...
	close FileGemo;
	if ($gemoerr > 0){
		die "ERROR in $dirname/runanalysis_2ndround_v2_genomic_withgff_gemoma: GeMoMa died with error running AF\n";
	}
	...
	close FileGemo;
	if ($gemoerr > 0){
		die "ERROR in $dirname/runanalysis_2ndround_v2_genomic_withgff_gemoma: GeMoMa died with error running CT\n";
	}
	...
	system ("cp $chem\/gemoma_outdir/$chem\_gemoma_genes_novel.gff3 $chem/$chem\_genomic_genes_unfiltered.gff3");
