...
# Script to confirm that translated proteins from GFF match with those in proteome file
...
while (<Blastfile>) {
	...
	push (@{$blast{$subline[0]}}, join("\t",$subline[1], $subline[2], $subline[4], $subline[10], $subline[12], $subline[13]));
}
close Blastfile;
