...
#usage: perl get_fasta_cut.pl tabular_file_with_positions_tocut Originalfasta.fasta outname
...
foreach my $key (sort keys %fasta){
	if (exists $inicio{$key}){
		...
		print "$key no lo ha sacado de la lista con las posiciones de inicio fin, algo va mal\n";
	}
